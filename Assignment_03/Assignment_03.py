...
# --- named constants (no magic numbers in the refactored solution) ---
TAX_RATE = 0.07
FOOD_TAX_RATE = 0.0
FOOD_CATEGORY = "food"
...
POINTS_DIVISOR = 10
RECEIPT_SEPARATOR = "-" * 40


class Product:
    """A store product with its own pricing and tax behaviour."""
    ...
    def __init__(self, name, price, category):
        ...
        self._tax_rate = FOOD_TAX_RATE if category == FOOD_CATEGORY else TAX_RATE

    def tax_for(self, amount):
        """Return tax for a given line amount."""
        return amount * self._tax_rate


class OrderItem:
    """An order line that has a Product and a quantity."""
...
class Customer:
    """Base customer tier; subclasses provide tier-specific behaviour."""
...
class Order:
    """An order has one Customer and many OrderItem objects."""
    ...
    def __init__(self, customer, items):
        ...
        if not all(isinstance(item, OrderItem) for item in items):
            raise TypeError("Order items must be OrderItem objects.")
    ...
    def discount(self):
        ...
        membership_discount = subtotal * self.customer.discount_rate(subtotal)
    ...
    def receipt(self):
        """Build receipt text separately from the calculation methods."""
        lines = [
